...
for skew in SKEW_LIST:
    try:
        BADGE_IMAGES[skew] = [f for f in os.listdir(f'/{BADGE_ASSETS_DIRECTORY}/{skew}') if f.endswith(".bin")]
...
for skew in SKEW_LIST:
    try:
        badge_text_files[skew] = [f for f in os.listdir(f'/{BADGE_ASSETS_DIRECTORY}/{skew}') if f.endswith(".txt")]
        if len(badge_text_files[skew]) > 1:
            print(f'Only one file can be used per skew. Found more than one file in skew {skew}')
...
company, name, detail1_title, detail1_text, detail2_title, detail2_text = dict(), dict(), dict(), dict(), dict(), dict()
for skew in SKEW_LIST:
...
for skew in SKEW_LIST:
    ...
    detail1_title[skew] = truncatestring(detail1_title[skew], DETAILS_TEXT_SIZE, TEXT_WIDTH)
    detail1_text[skew] = truncatestring(detail1_text[skew], DETAILS_TEXT_SIZE,
                                  TEXT_WIDTH - display.measure_text(detail1_title[skew], DETAILS_TEXT_SIZE))

    detail2_title[skew] = truncatestring(detail2_title[skew], DETAILS_TEXT_SIZE, TEXT_WIDTH)
    detail2_text[skew] = truncatestring(detail2_text[skew], DETAILS_TEXT_SIZE,
                                  TEXT_WIDTH - display.measure_text(detail2_title[skew], DETAILS_TEXT_SIZE))

# Compute spacing between detail text
display.font("sans")
distance_detail1 = TEXT_WIDTH - display.measure_text(detail1_title[skew]) - display.measure_text(detail1_text[skew])
distance_detail2 = TEXT_WIDTH - display.measure_text(detail2_title[skew]) - display.measure_text(detail2_text[skew])
...
def draw_badge(index: int = 0, skew: str = 'normal', full_update = True):
    ...
    open(f'{BADGE_ASSETS_DIRECTORY}/{skew}/{BADGE_IMAGES[skew][index]}', 'rb').readinto(badge_image)
    ...
    if full_update:
        ...
        display.thickness(1)
        display.text(company[skew], LEFT_PADDING + 10, (COMPANY_HEIGHT // 2) - 10, COMPANY_TEXT_SIZE + 2.4)
        ...
        while True:
            ...
            if name_length >= (TEXT_WIDTH - NAME_PADDING) and name_size >= 0.1:
                ...
                display.text(name[skew], (TEXT_WIDTH - name_length) // 2 + 5, (NAME_HEIGHT // 2) + COMPANY_HEIGHT - 33, name_size)
                break
        ...
        display.text(detail1_title[skew], LEFT_PADDING, HEIGHT - ((DETAILS_HEIGHT * 3) // 2), DETAILS_TEXT_SIZE)
        display.thickness(2)
        display.text(detail1_text[skew], LEFT_PADDING + name_length + 40, HEIGHT - ((DETAILS_HEIGHT * 3) // 2), DETAILS_TEXT_SIZE)

        # Draw the second detail's title and text
        display.thickness(2)
        name_length = display.measure_text(detail2_title[skew], DETAILS_TEXT_SIZE)
        display.text(detail2_title[skew], LEFT_PADDING, HEIGHT - (DETAILS_HEIGHT // 2), DETAILS_TEXT_SIZE)
        display.thickness(2)
        display.text(detail2_text[skew], LEFT_PADDING + name_length + 40, HEIGHT - (DETAILS_HEIGHT // 2), DETAILS_TEXT_SIZE)
